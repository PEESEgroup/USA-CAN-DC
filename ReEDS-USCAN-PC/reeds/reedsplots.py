...
def _ensure_2d_axes(ax, nrows, ncols):
    """Normalize matplotlib axes to a 2D ndarray for consistent indexing."""
    if nrows == 1 and ncols == 1:
        return np.array([[ax]], dtype=object)
    if nrows == 1:
        return np.array([ax], dtype=object)
    if ncols == 1:
        return np.array(ax, dtype=object).reshape(-1, 1)
    return np.array(ax, dtype=object)


def _ensure_1d_axes(ax):
    """Normalize matplotlib axes to a 1D ndarray for consistent indexing."""
    if isinstance(ax, np.ndarray):
        return ax.ravel()
    return np.array([ax], dtype=object)


def _ax_from_coords(ax, coord):
    """Return a single axes object from either tuple or flat subplot coordinates."""
    if isinstance(coord, tuple):
        return ax[coord]
    return np.asarray(ax, dtype=object).ravel()[coord]


def _converter_marker_sizes(values, wscale):
    """Build a 1D scatter-size array aligned to x/y coordinates."""
    sizes = np.asarray(values, dtype=float).reshape(-1)
    return (sizes * float(wscale)) ** 2 * np.pi / 4
...
def plot_trans_onecase(
    ...
    * If colors is a string instead of a dictionary, aggregate all transmission types
    ...
    if show_converters and ('VSC' in dfplot.trtype.unique()) and (not dfconverter.empty):
        ax.scatter(
            dfconverter.x.values, dfconverter.y.values,
            facecolor='none', edgecolor='k',
            s=_converter_marker_sizes(dfconverter.MW.values, wscale),
...
def plot_trans_vsc(
    ...
    ### Converter capacity
    if cmap and (not dfconverter.empty):
    ...
    if not dfconverter.empty:
        ax.scatter(
            dfconverter.x.values, dfconverter.y.values,
            facecolor='none', edgecolor='k',
            s=_converter_marker_sizes(dfconverter.GW.values, wscale),
            zorder=1E6,
        )
...
def map_net_imports(
    ...
    plt.close()
    f,ax = plt.subplots(
        _nrows, ncols, figsize=(13.33, 6.88), sharex=True, sharey=True,
        gridspec_kw={'hspace':-0.05, 'wspace':-0.05},
    )
    ax = _ensure_2d_axes(ax, _nrows, ncols)
    for year in years:
        year_ax = _ax_from_coords(ax, coords[year])
        df = dfmap[level].copy()
        ...
        ## Data
        df.plot(
            ax=year_ax, column='net_import', cmap=cmap,
            legend=False,
            vmin=-vmax[year], vmax=vmax[year],
        )
        ## Formatting
        year_ax.set_title(year, y=0.9)
        if vlim != 'shared':
            plots.addcolorbarhist(
                f=f, ax0=year_ax, data=df.net_import,
...
def plot_vresites_transmission(
    ...
    for tech in techs:
        try:
            ...
            cap[tech] = cap[tech].dropna(subset=['latitude', 'longitude']).copy()
            if cap[tech].empty:
                continue
...
def plot_retire_add(
    ...
    if peak:
        dfpeak = pd.read_csv(
            os.path.join(case,'inputs_case','peakload.csv'),
            index_col=['level','region'],
        ).rename(columns=int).loc['country', _years]
        if isinstance(dfpeak, pd.DataFrame):
            if 'USA' in dfpeak.index:
                dfpeak = dfpeak.loc['USA']
            elif len(dfpeak) == 1:
                dfpeak = dfpeak.iloc[0]
            else:
                dfpeak = dfpeak.sum(axis=0)
        dfpeak = dfpeak.astype(float) / 1e3
...
def plot_stressperiod_dispatch(case, tmin=2023, level='country', regions='USA'):
    ...
    ax = _ensure_2d_axes(ax, len(ts), ncols)
...
def plot_stressperiod_days(case, repcolor='k', sharey=False, figsize=(10,5)):
    ...
    plt.close()
    if not len(years):
        f, ax = plt.subplots(1, 1, figsize=figsize, sharex=True, sharey=sharey)
        return f, _ensure_1d_axes(ax)
    f,ax = plt.subplots(len(years), 1, figsize=figsize, sharex=True, sharey=sharey)
    ax = _ensure_1d_axes(ax)
...
def plot_stressperiod_evolution(
    ...
    available_metrics = sorted({
        str(m).lower()
        for df in dictin_neue.values()
        for m in df.index.get_level_values('metric').unique()
    })
    metric_key = metric_alias.get(str(metric).lower(), str(metric).lower())
    if metric_key not in available_metrics:
        metric_key = available_metrics[0] if available_metrics else metric_key
    ## Reshape to (year,iteration) x (region)
    dfplot = (
        pd.concat(dictin_neue, names=['year','iteration'])
        .xs(level,0,'level')
        .xs(metric_key,0,'metric')
    ...
    if not ncols or not len(regions):
...
def map_period_dispatch(
    ...
    * Currently only works for stress periods
    ...
    if level in _dfcenter.index.get_level_values('level'):
        ...
        fallback_centers = dfmap[level][['centroid_x','centroid_y']].rename(
            columns={'centroid_x':'x','centroid_y':'y'}
        )
        missing_regions = fallback_centers.index.difference(dfcenter.index)
        if len(missing_regions):
            dfcenter = pd.concat([dfcenter, fallback_centers.loc[missing_regions]], axis=0)
    else:
    ...
    if transmission:
        for interface in dftrans:
            r1, r2 = interface.split('|')
            if (r1, r2) in dfcorridors.index:
                x, y, angle = dfcorridors.loc[(r1, r2)]
            else:
                x = (dfcenter.loc[r1, 'x'] + dfcenter.loc[r2, 'x']) / 2
                y = (dfcenter.loc[r1, 'y'] + dfcenter.loc[r2, 'y']) / 2
                angle = np.arctan2(
                    dfcenter.loc[r1, 'y'] - dfcenter.loc[r2, 'y'],
                    dfcenter.loc[r1, 'x'] - dfcenter.loc[r2, 'x'],
                ) * 180 / np.pi
...
def plot_seed_stressperiods(
    ...
    if not years:
        ...
        ax.axis('off')
        ax.annotate(
            'No modeled years at or after GSw_StartMarkets',
