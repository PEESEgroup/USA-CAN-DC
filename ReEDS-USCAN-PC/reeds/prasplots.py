...
def plot_interface_flows(
    ...
    def _hierarchy_region(token):
        """Map PRAS interface endpoint labels to hierarchy regions."""
        if token in hierarchy.index:
            return hierarchy.loc[token, level]
        if '|' in token:
            candidate = token.split('|', 1)[1]
            if candidate in hierarchy.index:
                return hierarchy.loc[candidate, level]
    ...
    aggcols = {c: '→'.join([_hierarchy_region(i) for i in c.split('→')]) for c in df}
