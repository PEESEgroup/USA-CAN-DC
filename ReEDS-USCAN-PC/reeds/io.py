...
def get_hierarchy(case=None, original=False, country='USA'):
    ...
    if case and country == 'USA':
        try:
            sw = get_switches(case)
            if 'can' in str(sw.get('GSw_Region', '')).lower():
                country = None
        except Exception:
            pass
...
def get_us_can_pca_map(crs='ESRI:102008'):
    """Get BA polygons for USA and Canada."""
    dfba = gpd.read_file(
        os.path.join(reeds_path, 'inputs', 'shapefiles', 'US_CAN_MEX_PCA')
    )
    dfba['country'] = dfba['country'].str.lower()
    dfba = dfba.loc[dfba['country'].isin(['usa', 'can'])].copy()
    return dfba.set_index('rb').to_crs(crs)


def get_transmission_endpoints(crs='EPSG:4326'):
    """Get transmission endpoints, including Canada/Mexico centroids when available."""
    csv_path = os.path.join(
        reeds_path,
        'inputs',
        'shapefiles',
        'US_transmission_endpoints_and_CAN_MEX_centroids.csv',
    )
    if os.path.exists(csv_path):
        endpoints = pd.read_csv(csv_path)
        endpoints['geometry'] = endpoints['WKT'].map(wkt.loads)
        endpoints = gpd.GeoDataFrame(endpoints, geometry='geometry', crs='EPSG:4326')
        endpoints['node_longitude'] = endpoints.geometry.x
        endpoints['node_latitude'] = endpoints.geometry.y
        return endpoints.set_index('ba_str').to_crs(crs)

    endpoints = gpd.read_file(
        os.path.join(reeds_path, 'inputs', 'shapefiles', 'transmission_endpoints')
    ).set_index('ba_str').to_crs(crs)
    endpoints = endpoints.rename(columns={'lon': 'node_longitude', 'lat': 'node_latitude'})
    return endpoints
...
def get_zonemap(case=None, exclude_water_areas=False, crs='ESRI:102008'):
    ...
    if agglevel_variables['lvl'] == 'mult':
        ### Model zones
        dfba = get_us_can_pca_map(crs).reset_index()
        ### Use transmission endpoints from reV
        endpoints = get_transmission_endpoints(crs)
    ...
    ######## Single Resolution Procedure ########
        ...
        if sw.GSw_RegionResolution != 'county':
            ...
            ### Model zones
            dfba = get_us_can_pca_map(crs)[['geometry']].copy()
            ## Add transmission endpoints
            endpoints = (
                get_transmission_endpoints(crs)
    ...
    try:
        ...
        for i, row in asw.iterrows():
            ...
            if row['dtype'] == 'list':
                ...
                value = False if str(value).lower() == 'false' else True
            elif row['dtype'] == 'str':
                value = str(value)
            elif row['dtype'] == 'int':
                value = int(value)
                ...
                value = float(value)
            parsed_values[i] = value
        sw = pd.concat([sw, pd.Series(parsed_values, name='value')])
...
def get_outage_hourly(
    ...
    with h5py.File(os.path.join(inputs_case, f'outage_{outage_type}_hourly.h5'), 'r') as f:
        ...
        _index = pd.DatetimeIndex(
            pd.to_datetime(pd.Series(f['index']).map(lambda x: x.decode()))
        )
        if _index.tz is None:
            _index = _index.tz_localize('UTC')
        dfout = pd.DataFrame(
            index=_index,
            columns=columns,
            data=f['data'],
