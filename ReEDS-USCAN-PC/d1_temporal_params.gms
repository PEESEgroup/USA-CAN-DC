...
* Augur can add cumulative stress-period iterations after createmodel.gms has
* written the restart GDX. Reload the refreshed data-center timing inputs on
* every sequential/PCM solve so the current restart sees those new hours.
$onempty
Sets
datacenter_train_day(*) "24-hour model days used for daily training allocation"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_day_h(datacenter_train_day,allh) "timeslices in each data-center model day"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_route(*) "direct, cyclic training-task timing routes"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_route_source(datacenter_train_route,allh) "source timeslice for each direct training-task route"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_route_destination(datacenter_train_route,allh) "destination timeslice for each direct training-task route"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_route_rep_wrap(datacenter_train_route) "representative-day routes that cross the daily cycle boundary"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_route_stress_cross_day(datacenter_train_route) "stress-period routes that cross a natural-day boundary"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_route_stress_wrap(datacenter_train_route) "stress-period routes that close the period cycle"
/
$offlisting
$ondelim
...
Parameters
datacenter_infer_multiplier(r,allh) "--fraction-- local-time hourly inference workload multiplier"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_day_weight(datacenter_train_day) "--occurrences/year-- annual weight of each data-center model day"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_intraday_share(r,datacenter_train_day,allh) "--fraction-- source-region daily training profile"
/
$offlisting
$ondelim
...
$onlisting
/
datacenter_train_route_delay(datacenter_train_route) "--hours-- direct delay represented by each training-task route"
/
$offlisting
$ondelim
...
* -- Mexico and Canada --
*=============================================
parameter can_imports_szn_frac(allszn) "--unitless-- Legacy Canadian import season fractions (disabled)" ;
parameter can_exports_h_frac(allh) "--unitless-- Legacy Canadian export timeslice fractions (disabled)" ;

can_imports_szn_frac(allszn) = 0 ;
can_exports_h_frac(allh) = 0 ;
...
* Legacy exogenous Canada/Mexico load handling.
* Canada is now modeled endogenously, so Canadian regions should retain
* their load_allyear-based load_exog values. Only keep the old Mexico
* growth-based overwrite for Mexican regions when canmexload is provided.
load_exog(r,h,t)$[
    canmexload(r,h)
    $(not sum{
        country$[
            r_country(r,country)
            $(sameas(country,"USA") or sameas(country,"CAN"))
        ],
        1
    })
] = mex_growth_rate(t) * canmexload(r,h) ;
