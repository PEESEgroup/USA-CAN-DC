...
if REEDS_PATH not in sys.path:
    sys.path.append(REEDS_PATH)

CASE_PARENT_REPO = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if os.path.isdir(os.path.join(CASE_PARENT_REPO, 'postprocessing')) and CASE_PARENT_REPO not in sys.path:
    sys.path.append(CASE_PARENT_REPO)

import reeds
from postprocessing import calc_grid_carbon_water_factors
from postprocessing.retail_rate_module import calculate_historical_capex
...
def postprocess_outputs(case, outputs_path=None, verbose=0):
    ...
    ## Historical capex init file for downstream system cost postprocessing
    df_capex_init = os.path.join(case, 'inputs_case', 'df_capex_init.csv')
    if not os.path.exists(df_capex_init):
        try:
    ...
    ## Grid carbon and water factor outputs
    try:
        calc_grid_carbon_water_factors.main(case)
    except Exception:
        print(traceback.format_exc())
...
if __name__ == '__main__' and not hasattr(sys, 'ps1'):
    ...
    parser.add_argument(
        '--symbols',
        default='',
        help='Comma-separated list of output symbols to dump (empty means all)',
    )
    parser.add_argument(
        '--skip-postprocess',
        action='store_true',
        help='Skip python postprocessing outputs (systemcost, tech transmission, etc.)',
    )
