...
def check_compatibility(sw):
    ...
    if (
        ...
        raise NotImplementedError(
            'At least one of GSw_ClimateHydro, GSw_ClimateDemand, GSw_ClimateWater, '
    ...
    hierarchy_suffix = (
        ...
        else f"_{sw['GSw_HierarchyFile']}"
    )
    hierarchy = pd.read_csv(
        os.path.join(reeds_path, 'inputs', f'hierarchy{hierarchy_suffix}.csv')
    )
...
def setupEnvironment(
    ...
    for case in casenames:
        ...
        # Canada is now modeled endogenously; keep the legacy switch only as an
        # internal compatibility default for existing GAMS conditionals.
        case_switches = df_cases[case].to_dict()
        case_switches.setdefault('GSw_Canada', '0')
        shcom = ' --case=' + BatchName + "_" + case
        for i,v in case_switches.items():
        ...
        caseSwitches.append(case_switches)
...
def write_batch_script(
    ...
    with open(os.path.join(casedir, 'call_' + batch_case + ext), 'w+') as OPATH:
        ...
        if LINUXORMAC:
            OPATH.writelines("export OMP_NUM_THREADS=1\n")
        else:
            OPATH.writelines("set OMP_NUM_THREADS=1\n")
        OPATH.writelines('\n')
        ...
        OPATH.writelines(
            ...
            + f" --GSw_CarbonWaterFactorOnly={caseSwitches.get('GSw_CarbonWaterFactorOnly', 0)} \n"
        )
        OPATH.writelines(writescripterrorcheck("e_report.gms"))
        if not LINUXORMAC:
            OPATH.writelines("endlocal\n")
        OPATH.writelines(f'python {logger}\n')
        carbon_water_factor_only = int(caseSwitches.get('GSw_CarbonWaterFactorOnly', 0))
        if carbon_water_factor_only:
            OPATH.writelines(
                f'python e_report_dump.py {casedir} -c '
                '--symbols emit_r,gen_ann,water_consumption_ivrt '
                '--skip-postprocess\n\n'
            )
        else:
        ...
        if not carbon_water_factor_only:
        ...
        if int(caseSwitches['debug']) and (not carbon_water_factor_only):
