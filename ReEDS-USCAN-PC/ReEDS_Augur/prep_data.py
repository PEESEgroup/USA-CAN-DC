...
def get_vsc_converter_capacity_from_tran_cap(tran_cap):
    """Infer per-region converter capacity from VSC transmission endpoints."""
    if tran_cap.empty:
        return pd.Series(dtype=float, name='MW')

    try:
        vsc = tran_cap.xs('VSC', level='trtype')
    except KeyError:
        return pd.Series(dtype=float, name='MW')
    ...
    vsc = vsc.reset_index()
    vsc['pair_r'] = vsc[['r', 'rr']].min(axis=1)
    vsc['pair_rr'] = vsc[['r', 'rr']].max(axis=1)
    pair_cap = (
        vsc.groupby(['pair_r', 'pair_rr'], as_index=False)['MW']
        .max()
    )

    endpoint_cap = pd.concat([
        pair_cap.rename(columns={'pair_r': 'r'})[['r', 'MW']],
        pair_cap.rename(columns={'pair_rr': 'r'})[['r', 'MW']],
    ])
    return endpoint_cap.groupby('r')['MW'].sum().rename('MW')
...
def main(t, casedir, iteration=0):
    ...
    if 'ra_datacenter_load' in gdxreeds and len(gdxreeds['ra_datacenter_load']):
        ra_datacenter_load = (
            gdxreeds['ra_datacenter_load']
            .loc[gdxreeds['ra_datacenter_load']['t'] == t]
            .drop(columns='t')
            .set_index('r')
            .squeeze(1)
            .reindex(load.columns).fillna(0)
        ...
        ra_datacenter_load = pd.Series(0, index=load.columns)
    ...
    load_year = (
        load.loc[t]
        .add(can_exports, fill_value=0)
        .add(ra_datacenter_load, axis='columns')
    )
    ...
    ### Converter capacity: fall back to inferred VSC endpoint capacity from tran_cap
    ## whenever CAP_CONVERTER.l did not assign that region.
    cap_converter = (
        gdxreeds['cap_converter_filt'].set_index('r').rename(columns={'Value':'MW'})
        if not gdxreeds['cap_converter_filt'].empty
        else pd.DataFrame(columns=['MW']).rename_axis('r')
    )
    inferred_converter = get_vsc_converter_capacity_from_tran_cap(tran_cap).to_frame()
    if not inferred_converter.empty:
        missing_converter_regions = inferred_converter.index.difference(cap_converter.index)
        cap_converter = pd.concat([
            cap_converter,
            inferred_converter.loc[missing_converter_regions],
        ])
    csvout['cap_converter'] = cap_converter
    ...
    # If PRM is empty/unavailable, set to zero so ReEDS2PRAS skips max unit-size limits.
    prm_raw = (
        gdxreeds['prm'].loc[gdxreeds['prm'].t == t].set_index('r').Value
        if ('prm' in gdxreeds and not gdxreeds['prm'].empty)
        else pd.Series(dtype=float)
    )
    prm_raw = pd.to_numeric(prm_raw, errors='coerce')
    pras_peak = pd.to_numeric(h5out['pras_load'].max(), errors='coerce')
    if prm_raw.dropna().empty:
        max_unitsize = pd.Series(index=h5out['pras_load'].columns, data=0, dtype=int)
    else:
        max_unitsize = (
            prm_raw.mul(pras_peak, fill_value=np.nan)
            .pipe(pd.to_numeric, errors='coerce')
            .reindex(h5out['pras_load'].columns)
            .fillna(0)
            .round()
            .astype(int)
        )
    csvout['max_unitsize'] = max_unitsize.rename_axis('r').rename('mw')
