...
scalars
       Sw_DatacenterNoBuild "Block all data center construction" / %Sw_DatacenterNoBuild% /
       Sw_DatacenterDeploymentType "Data center siting mode" / %Sw_DatacenterDeploymentType% /
       Sw_DatacenterLayoutYear     "Last year before endogenous data center siting" / %Sw_DatacenterLayoutYear% /
       Sw_DatacenterEndogenousShare "Share of data center growth after the layout cutoff year that can be endogenously sited" / %Sw_DatacenterEndogenousShare% /
       Sw_DatacenterTrainTemporalFlexShare "Share of training workload that may execute later" / %Sw_DatacenterTrainTemporalFlexShare% /
       Sw_DatacenterTrainMaxDelayHours "Maximum direct cyclic training delay in real hours" / %Sw_DatacenterTrainMaxDelayHours% /
       Sw_DatacenterSpatialFlexScope "Data-center spatial flexibility scope" / %Sw_DatacenterSpatialFlexScope% /
       Sw_DatacenterSpatialFlexShare "Share of each task workload that may move across regions" / %Sw_DatacenterSpatialFlexShare% /
       Sw_DatacenterMovePenalty "Data-center workload movement penalty in 2004$/MWh-km" / %Sw_DatacenterMovePenalty% / ;
abort$[(Sw_DatacenterNoBuild <> 0) and (Sw_DatacenterNoBuild <> 1)]
    "Sw_DatacenterNoBuild must be 0 or 1." ;
abort$[(Sw_DatacenterDeploymentType <> 0) and (Sw_DatacenterDeploymentType <> 1) and (Sw_DatacenterDeploymentType <> 2)]
    "Sw_DatacenterDeploymentType must be 0, 1, or 2." ;
abort$[(Sw_DatacenterEndogenousShare < 0) or (Sw_DatacenterEndogenousShare > 1)]
    "Sw_DatacenterEndogenousShare must be between 0 and 1." ;
abort$[(Sw_DatacenterTrainTemporalFlexShare < 0) or (Sw_DatacenterTrainTemporalFlexShare > 1)]
    "Sw_DatacenterTrainTemporalFlexShare must be between 0 and 1." ;
abort$[(Sw_DatacenterSpatialFlexShare < 0) or (Sw_DatacenterSpatialFlexShare > 1)]
    "Sw_DatacenterSpatialFlexShare must be between 0 and 1." ;
abort$[(Sw_DatacenterTrainMaxDelayHours < 0) or (Sw_DatacenterTrainMaxDelayHours >= 24) or (Sw_DatacenterTrainMaxDelayHours <> floor(Sw_DatacenterTrainMaxDelayHours))]
    "Sw_DatacenterTrainMaxDelayHours must be an integer between 0 and 23." ;
abort$[(Sw_DatacenterSpatialFlexScope <> 0) and (Sw_DatacenterSpatialFlexScope <> 1) and (Sw_DatacenterSpatialFlexScope <> 2)]
    "Sw_DatacenterSpatialFlexScope must be 0, 1, or 2." ;
abort$[Sw_DatacenterMovePenalty < 0]
    "Sw_DatacenterMovePenalty must be non-negative." ;
...
canus_pool "Canada-US pooling regions"
/
* written by copy_files.py
$include inputs_case%ds%val_canus_pool.csv
/,

whole_pool "Whole-system pooling regions"
/
* written by copy_files.py
$include inputs_case%ds%val_whole_pool.csv
/,
...
* Apply regional technology bans to cooling-water variants derived from banned
* numeraire technologies, e.g. coal-new -> coal-new_r_fsa.
tech_banned(i,r)$[Sw_WaterMain$sum{ii$ctt_i_ii(i,ii), tech_banned(ii,r)}] = yes ;
...
set hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) "hierarchy of various regional definitions"
/
$offlisting
$ondelim
...
set r_itlgrp(r,itlgrp)
    ...
    r_canus_pool(r,canus_pool)
    r_whole_pool(r,whole_pool)
    r_country(r,country)
...
r_nercr(r,nercr)                      $sum{(      transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_transreg(r,transreg)                $sum{(nercr,         transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_transgrp(r,transgrp)                $sum{(nercr,transreg,         cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_cendiv(r,cendiv)                    $sum{(nercr,transreg,transgrp,       st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_st(r,st)                            $sum{(nercr,transreg,transgrp,cendiv,   interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_interconnect(r,interconnect)        $sum{(nercr,transreg,transgrp,cendiv,st,             canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_canus_pool(r,canus_pool)            $sum{(nercr,transreg,transgrp,cendiv,st,interconnect,           whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_whole_pool(r,whole_pool)            $sum{(nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,           country,usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_country(r,country)                  $sum{(nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,        usda_region,h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_usda(r,usda_region)                 $sum{(nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,            h2ptcreg,hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_h2ptcreg(r,h2ptcreg)                $sum{(nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,         hurdlereg,ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_hurdlereg(r,hurdlereg)              $sum{(nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,          ccreg) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
r_ccreg(r,ccreg)                      $sum{(nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg      ) $hierarchy(r,nercr,transreg,transgrp,cendiv,st,interconnect,canus_pool,whole_pool,country,usda_region,h2ptcreg,hurdlereg,ccreg),1} = yes ;
...
*==========================================
* Canada is now modeled endogenously in this branch, so keep the legacy
* exogenous interface defined as zero for compatibility only.
$onempty
parameter can_imports(r,allt) "--MWh-- Legacy exogenous imports from Canada (disabled)"
/ / ;

parameter can_imports_capacity(r,allt) "--MW-- Legacy exogenous Canadian import capacity (disabled)"
/ / ;

parameter can_exports(r,allt) "--MWh-- Legacy exogenous exports to Canada (disabled)"
/ / ;
$offempty
...
* Legacy exogenous Canadian imports are disabled when Canada is endogenous.
capacity_exog("can-imports","init-1",r,t) = 0 ;
...
parameter cap_converter_init(r) "--MW-- initial VSC AC/DC converter capacity"
/
$offlisting
$ondelim
$include inputs_case%ds%cap_converter_init.csv
$offdelim
$onlisting
/ ;
$offempty
...
$onempty
parameter cap_converter_fut_raw(r,allt) "--MW-- certain future VSC converter capacity at commissioning year"
/
$offlisting
$ondelim
$include inputs_case%ds%cap_converter_fut.csv
$offdelim
$onlisting
/ ;
$offempty

parameter cap_converter_fut(r,allt) "--MW-- cumulative certain future VSC converter capacity by year" ;
cap_converter_fut(r,t) = sum{alltt$[year(alltt) <= yeart(t)], cap_converter_fut_raw(r,alltt)} ;
...
* Apply to all "certain" future transmission projects, not just VSC.
* Map each project to the first modeled year at or after its commissioning year
* so off-cycle additions (e.g. 2026 in a 2010/2020/2030 run) are forced exactly
* once. Projects commissioned before the first model year are excluded (they should
* appear in trancap_init, not trancap_fut).
invtran_exog(r,rr,trtype,t)$tmodel_new(t) =
    sum{alltt$[year(alltt) <= yeart(t)
              $(year(alltt) >= sum{tt$tfirst(tt), yeart(tt)})
              $(not sum{tt$[tmodel_new(tt)$(yeart(tt) < yeart(t))$(yeart(tt) >= year(alltt))], 1 })],
        trancap_fut(r,rr,"certain",trtype,alltt) } ;
...
* invtran_exog above applies to all certain transmission projects.
* Both blocks below restore routes_inv for exogenous corridors that were blocked by
* the timing and TransRestrict filters above. Both directions are set explicitly
* because the symmetry line (routes_inv(rr,r)$[not routes_inv(r,rr)]=no) ran before
* the TransRestrict block, so the reverse direction may have been cleared.
* VSC routes do not use the notvsc default routes_inv logic, so they need a
* separate block.
routes_inv(r,rr,trtype,t)$[sameas(trtype,'VSC')$invtran_exog(r,rr,trtype,t)] = yes ;
routes_inv(rr,r,trtype,t)$[sameas(trtype,'VSC')$invtran_exog(r,rr,trtype,t)] = yes ;
routes_inv(r,rr,trtype,t)$[notvsc(trtype)$invtran_exog(r,rr,trtype,t)] = yes ;
routes_inv(rr,r,trtype,t)$[notvsc(trtype)$invtran_exog(r,rr,trtype,t)] = yes ;
...
* Also allow converter operation at BAs that have existing VSC connections but no future investment route
val_converter(r,t)$[sum{rr, trancap_init_energy(r,rr,"VSC") + trancap_init_energy(rr,r,"VSC")}] = yes ;
* Also allow converter operation at BAs that receive certain future VSC converter capacity in this year or earlier
val_converter(r,t)$[cap_converter_fut(r,t)] = yes ;
...
$onempty
parameter datacenter_pue(st,allt) "--unitless-- data center power usage effectiveness by state/province and year"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_pue.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_workload_param "annual data-center workload parameter"
/ train_share, infer_share, train_util, infer_util / ;

parameter datacenter_train_infer_util(allt,datacenter_workload_param) "--fraction-- annual training/inference workload shares and utilisations"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_infer_util.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_spatial_route(r,rr) "allowed directed data-center workload routes"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_spatial_route.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_move_distance(r,rr) "--km-- BA-anchor shortest-path proxy for data movement"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_move_distance.csv
$offdelim
$onlisting
/ ;

scalars
    datacenter_rho_idle "--fraction-- idle AI-server power relative to nameplate" / 0.23 /
    datacenter_rho_max "--fraction-- active AI-server power relative to nameplate" / 0.88 / ;

$onempty
parameter datacenter_wue(st,allt) "--L/kWh-- direct data center water usage effectiveness by state/province and year"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_wue.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_water_cost(st) "--2004$/Mgal/yr-- average fresh surface appropriated water access cost by state/province"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_water_cost.csv
$offdelim
$onlisting
/ ;

* dc61 is an uncapped backstop bin appended by input_processing/datacenter.py
* beyond the 60-bin ascending-cost curve read from ba_datacenter_supply_curve.csv;
* it absorbs siting demand once the priced curve is fully deployed.
set datacenter_sc_bin "data center supply curve bins" / dc1*dc61 / ;
alias(datacenter_sc_bin,datacenter_sc_bin2) ;

set datacenter_sc_bin_uncapped(datacenter_sc_bin) "data-center supply-curve bin with no capacity limit, used once the ascending-cost curve (dc1-dc60) is fully deployed"
/ dc61 / ;

$onempty
table datacenter_sc_input(r,datacenter_sc_bin,*) "data center supply-curve capacity and CAPEX by modeled region and bin"
$offlisting
$ondelim
$include inputs_case%ds%datacenter_supply_curve.csv
$offdelim
$onlisting
;
$offempty

parameter datacenter_sc_capacity(r,datacenter_sc_bin) "--MW-- data center supply-curve capacity by modeled region and bin"
          datacenter_sc_capex(r,datacenter_sc_bin) "--2004$/MW-- data center overnight CAPEX by modeled region and bin"
          datacenter_sc_capex_2024(r,datacenter_sc_bin) "--2024$/MW-- source data center overnight CAPEX by modeled region and bin" ;
datacenter_sc_capacity(r,datacenter_sc_bin) = datacenter_sc_input(r,datacenter_sc_bin,"capacity_mw") ;
datacenter_sc_capex(r,datacenter_sc_bin) = datacenter_sc_input(r,datacenter_sc_bin,"capex_2004usd_per_mw") ;
datacenter_sc_capex_2024(r,datacenter_sc_bin) = datacenter_sc_input(r,datacenter_sc_bin,"capex_2024usd_per_mw") ;

* A region with zero capacity across every priced bin (dc1-dc60) has no real
* supply-curve data at all (e.g. it is missing from ba_datacenter_supply_curve.csv
* or its BA-to-region mapping). Its uncapped backstop bin (dc61) must not be
* usable either -- otherwise it would be a zero-cost, unconstrained investment
* outlet for that region regardless of whether it actually has siting data.
datacenter_sc_capacity(r,datacenter_sc_bin)$[
    datacenter_sc_bin_uncapped(datacenter_sc_bin)
    and (not sum{datacenter_sc_bin2$(not datacenter_sc_bin_uncapped(datacenter_sc_bin2)),
                 datacenter_sc_capacity(r,datacenter_sc_bin2) })
] = 0 ;

* datacenter_sc_exog_inv(r,bin,t) is *not* loaded from a precomputed CSV: a
* one-time preprocessing pass (before any solve) has no way to know how much
* of a bin's capacity endogenous investment (INV_DATACENTER_SC) will
* separately claim in earlier solve years, so pre-assigning bins here can
* crowd out capacity that a later year's exogenous requirement is already
* known to need. Instead it is filled one model year at a time in
* d1_datacenter_sc_fill.gms (included from d_solveoneyear.gms, before that
* year's solve), using each bin's capacity actually still available after
* the region's realized (solved) endogenous+exogenous occupancy in prior
* years -- tracked in datacenter_sc_occupied_total below.
parameter datacenter_sc_exog_inv(r,datacenter_sc_bin,t) "--MW-- retained post-layout exogenous data center growth assigned to a supply-curve bin, filled one model year at a time by d1_datacenter_sc_fill.gms" ;

parameter datacenter_sc_occupied_total(r,datacenter_sc_bin) "--MW-- cumulative realized (solved) endogenous+exogenous occupancy of a supply-curve bin through all tfix'd years, updated in d2_post_solve_adjustments.gms" ;
datacenter_sc_occupied_total(r,datacenter_sc_bin) = 0 ;

* Declared here (not in d1_datacenter_sc_fill.gms, which is $included fresh
* once per solve year) so re-including that file in a later year's restarted
* job does not attempt to redeclare it.
parameter datacenter_sc_exog_remaining(r) "--MW-- current model year's retained-exogenous requirement increment not yet assigned to a bin, used transiently by d1_datacenter_sc_fill.gms" ;

$onempty
parameter datacenter_sc_exog_req(r,t) "--MW-- region-level cumulative retained post-layout exogenous data-center growth requirement, not yet assigned to a bin"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_sc_exog_req.csv
$offdelim
$onlisting
/ ;
$offempty
...
$onempty
parameter datacenter_cap_cost_mult(allt) "--unitless-- financial multiplier for 40-year data center capital costs"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_cap_cost_mult.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_it_exog(st,allt) "--MW-- exogenous cumulative data center IT load by state/province"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_it_exog.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_it_min(st,allt) "--MW-- data center IT load floor through the layout cutoff year by state/province"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_it_min.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_country_it_exog(country,allt) "--MW-- cumulative data center IT load by country"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_country_it_exog.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_total_it_exog(allt) "--MW-- cumulative data center IT load across countries"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_total_it_exog.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_r_st_ratio(r,st) "--fraction-- share of state/province data center load assigned to each BA"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_r_st_ratio.csv
$offdelim
$onlisting
/ ;

set datacenter_st(st) "states/provinces with data center input data" ;
datacenter_st(st)$[
    (not Sw_DatacenterNoBuild)
    and sum{t, datacenter_pue(st,t)}
    and (
        (Sw_DatacenterDeploymentType > 0)
        or sum{t, datacenter_it_exog(st,t) + datacenter_it_min(st,t) }
    )
] = yes ;

set datacenter_r(r) "regions eligible for data center load" ;
* Every modeled BA in a state/province with data-center inputs is eligible for
* endogenous siting.  datacenter_r_st_ratio is retained only to place the
* exogenous path and the non-relocatable floor across BAs.
datacenter_r(r)$sum{st$[datacenter_st(st)$r_st(r,st)], 1 } = yes ;

* Workload may only be executed in regions that participate in the data-center
* model.  The preprocessing route file covers the full spatial hierarchy because
* datacenter_r depends on siting switches and GAMS inputs; filter it here after
* datacenter_r has been constructed so zero-capacity candidate regions remain
* available while regions outside the data-center model cannot absorb workload.
datacenter_spatial_route(r,rr)$[
    (not datacenter_r(r)) or (not datacenter_r(rr))
] = no ;

parameter datacenter_it_exog_r(r,allt) "--MW-- exogenous data center IT load by BA" ;
parameter datacenter_it_min_r(r,allt) "--MW-- non-relocatable data center IT load by BA" ;
parameter datacenter_r_st_weight(r,st,allt) "--fraction-- share of a region's data center IT load assigned to each state/province" ;
parameter datacenter_r_ratio_total(r) "--fraction-- sum of state/province allocation ratios associated with a region" ;
parameter datacenter_pue_r(r,allt) "--unitless-- effective data center PUE by region and year" ;
datacenter_it_exog_r(r,allt)$datacenter_r(r) =
    sum{st$datacenter_r_st_ratio(r,st),
        datacenter_r_st_ratio(r,st) * datacenter_it_exog(st,allt) } ;
datacenter_it_min_r(r,allt)$datacenter_r(r) =
    sum{st$datacenter_r_st_ratio(r,st),
        datacenter_r_st_ratio(r,st) * datacenter_it_min(st,allt) } ;
datacenter_r_ratio_total(r)$datacenter_r(r) =
    sum{st$datacenter_r_st_ratio(r,st), datacenter_r_st_ratio(r,st) } ;
datacenter_r_st_weight(r,st,allt)$[
    datacenter_r(r)
    $datacenter_r_st_ratio(r,st)
    $datacenter_it_exog_r(r,allt)
] = datacenter_r_st_ratio(r,st) * datacenter_it_exog(st,allt)
    / datacenter_it_exog_r(r,allt) ;
* Regions with no exogenous IT load in a year retain a valid PUE for possible
* endogenous siting by falling back to normalized allocation ratios.
datacenter_r_st_weight(r,st,allt)$[
    datacenter_r(r)
    $datacenter_r_st_ratio(r,st)
    $(not datacenter_it_exog_r(r,allt))
    $datacenter_r_ratio_total(r)
] = datacenter_r_st_ratio(r,st) / datacenter_r_ratio_total(r) ;
* Some BAs have a zero exogenous allocation ratio because their state's
* exogenous data-center path is assigned to a different BA.  They are still
* eligible for endogenous siting through datacenter_r(r).  Give those BAs a
* state/province mapping so endogenous capacity has a PUE, contributes load,
* and is included in state/province reporting instead of becoming invisible.
datacenter_r_st_weight(r,st,allt)$[
    datacenter_r(r)
    $r_st(r,st)
    $(not datacenter_r_ratio_total(r))
] = 1 / sum{ast$[datacenter_st(ast)$r_st(r,ast)], 1 } ;
datacenter_pue_r(r,allt)$datacenter_r(r) =
    sum{st$datacenter_r_st_weight(r,st,allt),
        datacenter_r_st_weight(r,st,allt) * datacenter_pue(st,allt) } ;

set datacenter_st_country(st,country) "data center state/province to country mapping" ;
datacenter_st_country(st,country)$[datacenter_st(st)$sum{r$[r_st(r,st)$r_country(r,country)], 1 }] = yes ;

set datacenter_country(country) "countries with data center input data" ;
datacenter_country(country)$sum{st$datacenter_st_country(st,country), 1 } = yes ;
...
* Data-center trace and daily training inputs use the allh domain, so they
* cannot be read until allh itself has been populated above.
$onempty
parameter datacenter_infer_multiplier(r,allh) "--fraction-- local-time hourly inference workload multiplier"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_infer_multiplier.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_day(*) "24-hour model days used for daily training allocation"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_day.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_day_h(datacenter_train_day,allh) "timeslices in each data-center model day"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_day_h.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_route(*) "direct, cyclic training-task timing routes"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_route.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_route_source(datacenter_train_route,allh) "source timeslice for each direct training-task route"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_route_source.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_route_destination(datacenter_train_route,allh) "destination timeslice for each direct training-task route"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_route_destination.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_route_rep_wrap(datacenter_train_route) "representative-day routes that cross the daily cycle boundary"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_route_rep_wrap.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_route_stress_cross_day(datacenter_train_route) "stress-period routes that cross a natural-day boundary"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_route_stress_cross_day.csv
$offdelim
$onlisting
/ ;

$onempty
set datacenter_train_route_stress_wrap(datacenter_train_route) "stress-period routes that close the period cycle"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_route_stress_wrap.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_train_day_weight(datacenter_train_day) "--occurrences/year-- annual weight of each data-center model day"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_day_weight.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_train_intraday_share(r,datacenter_train_day,allh) "--fraction-- source-region daily training profile"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_intraday_share.csv
$offdelim
$onlisting
/ ;

$onempty
parameter datacenter_train_route_delay(datacenter_train_route) "--hours-- direct delay represented by each training-task route"
/
$offlisting
$ondelim
$include inputs_case%ds%datacenter_train_route_delay.csv
$offdelim
$onlisting
/ ;
