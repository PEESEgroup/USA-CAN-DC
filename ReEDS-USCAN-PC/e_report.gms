...
loadtype "categories for types of load" / end_use, dist_loss, trans_loss, stor_charge, h2_prod, h2_network, dac, datacenter /
...
load_rt(r,t)$tmodel_new(t) =
  sum{h, hours(h) * (
      load_exog(r,h,t)
      + (
              datacenter_pue_r(r,t) * (
                  datacenter_rho_idle * CAP_DATACENTER.l(r,t)
              + DATACENTER_TRAIN.l(r,h,t)
              + DATACENTER_INFER.l(r,h,t)
            )
        )$datacenter_r(r)
  ) } ;
...
repgasquant(cendiv,t)$[abs(repgasquant(cendiv,t)) < 1e-12] = 0 ;
repgasquant_irt(i,r,t)$[abs(repgasquant_irt(i,r,t)) < 1e-12] = 0 ;
repgasquant_nat(t)$[abs(repgasquant_nat(t)) < 1e-12] = 0 ;
...
* Data-center siting supply-curve investment
systemcost_ba("inv_datacenter_siting",r,t)$[
    tmodel_new(t)
    $datacenter_r(r)
    $(Sw_DatacenterDeploymentType>0)
    $(yeart(t)>Sw_DatacenterLayoutYear)
] =
    sum{datacenter_sc_bin,
        datacenter_cap_cost_mult(t)
        * datacenter_sc_capex(r,datacenter_sc_bin)
        * INV_DATACENTER_SC.l(r,datacenter_sc_bin,t) }
;
...
datacenter_it_cap(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  sum{r$[datacenter_r(r)$datacenter_r_st_weight(r,st,t)],
      datacenter_r_st_weight(r,st,t) * CAP_DATACENTER.l(r,t) } ;
datacenter_it_inv(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  sum{r$[datacenter_r(r)$datacenter_r_st_weight(r,st,t)],
      datacenter_r_st_weight(r,st,t) * INV_DATACENTER.l(r,t) } ;
* Supply-curve deployment and raw source-cost outputs intentionally use the
* original 2024$/MW values without dollar-year or capital-recovery adjustments.
* In endogenous modes, report optimized endogenous investment plus deterministic
* ascending-cost retained-exogenous bin occupancy. In fixed mode, retain the
* ascending-cost attribution of the complete regional capacity layout after the
* layout cutoff year.
* Capacity through the cutoff year is neither charged nor allowed to consume
* the low-cost portion of the supply curve. These raw diagnostics are separate
* from the optimized endogenous investment cost used in the objective.
datacenter_sc_inv_out(r,datacenter_sc_bin,t)$[
    tmodel_new(t)
    $datacenter_r(r)
    $(yeart(t)>Sw_DatacenterLayoutYear)
    $(Sw_DatacenterDeploymentType>0)
] = INV_DATACENTER_SC.l(r,datacenter_sc_bin,t)
    + datacenter_sc_exog_inv(r,datacenter_sc_bin,t) ;
datacenter_sc_inv_out(r,datacenter_sc_bin,t)$[
    tmodel_new(t)
    $datacenter_r(r)
    $(yeart(t)>Sw_DatacenterLayoutYear)
  ...
  max(0,
      min(datacenter_sc_capacity(r,datacenter_sc_bin),
          CAP_DATACENTER.l(r,t)
          - datacenter_it_min_r(r,t)
          - sum{datacenter_sc_bin2$[
                    ord(datacenter_sc_bin2)<ord(datacenter_sc_bin)],
                datacenter_sc_capacity(r,datacenter_sc_bin2) }))
  - sum{tt$tprev(t,tt),
      max(0,
          min(datacenter_sc_capacity(r,datacenter_sc_bin),
              CAP_DATACENTER.l(r,tt)
              - datacenter_it_min_r(r,tt)
              - sum{datacenter_sc_bin2$[
                        ord(datacenter_sc_bin2)<ord(datacenter_sc_bin)],
                    datacenter_sc_capacity(r,datacenter_sc_bin2) })) } ;
datacenter_siting_cost_2024_r(r,t)$[
    tmodel_new(t)
    $datacenter_r(r)
    $(yeart(t)>Sw_DatacenterLayoutYear)
] =
  sum{datacenter_sc_bin,
      datacenter_sc_capex_2024(r,datacenter_sc_bin)
      * datacenter_sc_inv_out(r,datacenter_sc_bin,t) } ;
datacenter_siting_cost_2024(t)$[
    tmodel_new(t)
    $(yeart(t)>Sw_DatacenterLayoutYear)
] =
  sum{r$datacenter_r(r), datacenter_siting_cost_2024_r(r,t) } ;
* Baseline workload remains available for comparison with the realized schedule.
datacenter_train_workload_st_base(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  sum{(r,h)$[datacenter_r(r)$datacenter_r_st_weight(r,st,t)$h_rep(h)],
      hours(h) * datacenter_r_st_weight(r,st,t) * DATACENTER_BASE_TRAIN.l(r,h,t) } / 8760 ;
datacenter_infer_workload_st_base(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  sum{(r,h)$[datacenter_r(r)$datacenter_r_st_weight(r,st,t)$h_rep(h)],
      hours(h) * datacenter_r_st_weight(r,st,t) * DATACENTER_BASE_INFER.l(r,h,t) } / 8760 ;
datacenter_server_load_st_base(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  datacenter_rho_idle * datacenter_it_cap(st,t)
  + datacenter_train_workload_st_base(st,t) + datacenter_infer_workload_st_base(st,t) ;
datacenter_load_st_base(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  datacenter_pue(st,t) * datacenter_server_load_st_base(st,t) ;
datacenter_train_workload_st(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  sum{(r,h)$[datacenter_r(r)$datacenter_r_st_weight(r,st,t)$h_rep(h)],
      hours(h) * datacenter_r_st_weight(r,st,t) * DATACENTER_TRAIN.l(r,h,t) } / 8760 ;
datacenter_infer_workload_st(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  sum{(r,h)$[datacenter_r(r)$datacenter_r_st_weight(r,st,t)$h_rep(h)],
      hours(h) * datacenter_r_st_weight(r,st,t) * DATACENTER_INFER.l(r,h,t) } / 8760 ;
datacenter_server_load_st(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  datacenter_rho_idle * datacenter_it_cap(st,t)
  + datacenter_train_workload_st(st,t) + datacenter_infer_workload_st(st,t) ;
datacenter_load_st(st,t)$[tmodel_new(t)$datacenter_st(st)] =
  datacenter_pue(st,t) * datacenter_server_load_st(st,t) ;
datacenter_train_workload_r_base(r,t)$tmodel_new(t) =
  (sum{h$h_rep(h), hours(h) * DATACENTER_BASE_TRAIN.l(r,h,t) } / 8760)$datacenter_r(r) ;
datacenter_infer_workload_r_base(r,t)$tmodel_new(t) =
  (sum{h$h_rep(h), hours(h) * DATACENTER_BASE_INFER.l(r,h,t) } / 8760)$datacenter_r(r) ;
datacenter_server_load_r_base(r,t)$tmodel_new(t) =
  (datacenter_rho_idle * CAP_DATACENTER.l(r,t)
   + datacenter_train_workload_r_base(r,t) + datacenter_infer_workload_r_base(r,t))$datacenter_r(r) ;
datacenter_load_r_base(r,t)$tmodel_new(t) =
  datacenter_pue_r(r,t) * datacenter_server_load_r_base(r,t) ;
datacenter_train_workload_r(r,t)$tmodel_new(t) =
  (sum{h$h_rep(h), hours(h) * DATACENTER_TRAIN.l(r,h,t) } / 8760)$datacenter_r(r) ;
datacenter_infer_workload_r(r,t)$tmodel_new(t) =
  (sum{h$h_rep(h), hours(h) * DATACENTER_INFER.l(r,h,t) } / 8760)$datacenter_r(r) ;
datacenter_server_load_r(r,t)$tmodel_new(t) =
  (datacenter_rho_idle * CAP_DATACENTER.l(r,t)
   + datacenter_train_workload_r(r,t) + datacenter_infer_workload_r(r,t))$datacenter_r(r) ;
datacenter_load_r(r,t)$tmodel_new(t) = datacenter_pue_r(r,t) * datacenter_server_load_r(r,t) ;
datacenter_train_moved_r(r,t)$tmodel_new(t) =
  sum{(datacenter_train_day,rr)$[datacenter_spatial_route(r,rr)$(not sameas(r,rr))
      $sum{h$[datacenter_train_day_h(datacenter_train_day,h)$h_rep(h)], 1}],
      datacenter_train_day_weight(datacenter_train_day)
      * DATACENTER_TRAIN_SPATIAL.l(r,datacenter_train_day,rr,t) } ;
datacenter_train_spatial_annual(r,datacenter_train_day,rr,t)
    $[tmodel_new(t)$datacenter_spatial_route(r,rr)
      $sum{h$[datacenter_train_day_h(datacenter_train_day,h)$h_rep(h)], 1}] =
  datacenter_train_day_weight(datacenter_train_day)
  * DATACENTER_TRAIN_SPATIAL.l(r,datacenter_train_day,rr,t) ;
datacenter_train_temporal_shift_h(r,allh,t)
    $[tmodel_new(t)$datacenter_r(r)$h(allh)
      $sum{datacenter_train_day$datacenter_train_day_h(datacenter_train_day,allh), 1}] =
  hours_daily(allh)
  * (DATACENTER_TRAIN.l(r,allh,t) - DATACENTER_TRAIN_REFERENCE.l(r,allh,t)) ;
datacenter_train_flexible_energy_r(r,t)$tmodel_new(t) =
  Sw_DatacenterTrainTemporalFlexShare
  * sum{(datacenter_train_day,h)$[datacenter_train_day_h(datacenter_train_day,h)$h_rep(h)],
      datacenter_train_day_weight(datacenter_train_day)
      * hours_daily(h) * DATACENTER_TRAIN_REFERENCE.l(r,h,t) } ;
datacenter_train_rep_wrap_energy_r(r,t)$tmodel_new(t) =
  sum{(datacenter_train_route,datacenter_train_day,h)$[
      datacenter_train_route_rep_wrap(datacenter_train_route)
      $datacenter_train_route_source(datacenter_train_route,h)
      $datacenter_train_day_h(datacenter_train_day,h)],
      datacenter_train_day_weight(datacenter_train_day)
      * DATACENTER_TRAIN_FLEX_FLOW.l(r,datacenter_train_route,t) } ;
datacenter_train_stress_cross_day_energy_r(r,t)$tmodel_new(t) =
  sum{(datacenter_train_route,datacenter_train_day,h)$[
      datacenter_train_route_stress_cross_day(datacenter_train_route)
      $datacenter_train_route_source(datacenter_train_route,h)
      $datacenter_train_day_h(datacenter_train_day,h)],
      datacenter_train_day_weight(datacenter_train_day)
      * DATACENTER_TRAIN_FLEX_FLOW.l(r,datacenter_train_route,t) } ;
datacenter_train_stress_wrap_energy_r(r,t)$tmodel_new(t) =
  sum{(datacenter_train_route,datacenter_train_day,h)$[
      datacenter_train_route_stress_wrap(datacenter_train_route)
      $datacenter_train_route_source(datacenter_train_route,h)
      $datacenter_train_day_h(datacenter_train_day,h)],
      datacenter_train_day_weight(datacenter_train_day)
      * DATACENTER_TRAIN_FLEX_FLOW.l(r,datacenter_train_route,t) } ;
datacenter_train_max_delay_used_r(r,t)$tmodel_new(t) =
  smax{datacenter_train_route$[DATACENTER_TRAIN_FLEX_FLOW.l(r,datacenter_train_route,t) > rhs_tolerance],
      datacenter_train_route_delay(datacenter_train_route) } ;
datacenter_move_cost_r(r,t)$tmodel_new(t) = Sw_DatacenterMovePenalty
  * sum{(datacenter_train_day,rr)$[datacenter_spatial_route(r,rr)$(not sameas(r,rr))
      $sum{h$[datacenter_train_day_h(datacenter_train_day,h)$h_rep(h)], 1}],
      datacenter_train_day_weight(datacenter_train_day)
      * datacenter_move_distance(r,rr)
      * DATACENTER_TRAIN_SPATIAL.l(r,datacenter_train_day,rr,t) } ;
*=========================
...
load_cat("datacenter",r,t)$tmodel_new(t) =
  8760 * datacenter_load_r(r,t) ;
