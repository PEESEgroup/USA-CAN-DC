...
* --- investment costs ---
                  ...
                  + sum{(r,datacenter_sc_bin)$[
                       ...
                       * INV_DATACENTER_SC(r,datacenter_sc_bin,t)
                      }
...
* --- data center direct water consumption access cost ---
              + sum{(r,st,h)$[datacenter_r(r)$datacenter_r_st_weight(r,st,t)$h_rep(h)],
                   (1000 / 3.785411784E6) * hours(h)
                   * datacenter_wue(st,t) * datacenter_water_cost(st)
                   * datacenter_r_st_weight(r,st,t)
                   * (datacenter_rho_idle * CAP_DATACENTER(r,t)
                      + DATACENTER_TRAIN(r,h,t)
                      + DATACENTER_INFER(r,h,t)) }

* --- penalty for daily training workload moved across regions ---
              + sum{(r,rr,datacenter_train_day)$[datacenter_spatial_route(r,rr)
                    $(not sameas(r,rr))
                    $sum{h$datacenter_train_day_h(datacenter_train_day,h), 1}],
                   Sw_DatacenterMovePenalty * datacenter_move_distance(r,rr)
