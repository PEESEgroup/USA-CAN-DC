...
*   PEAK_FLEX(r,ccseason,t)          "--MW-- peak busbar load adjustment based on load flexibility"
  ...
  CAP_DATACENTER(r,t)             "--MW-- cumulative data center IT load by BA"
  INV_DATACENTER(r,t)             "--MW-- data center IT load added in model year by BA"
  INV_DATACENTER_SC(r,datacenter_sc_bin,t) "--MW-- endogenous data center IT load added from a supply-curve bin"
  DATACENTER_BASE_TRAIN(r,allh,t) "--MW-- baseline training workload intensity on the IT-power basis"
  DATACENTER_BASE_INFER(r,allh,t) "--MW-- baseline inference workload intensity on the IT-power basis"
  DATACENTER_TRAIN(r,allh,t)      "--MW-- realized training workload intensity on the IT-power basis"
  DATACENTER_INFER(r,allh,t)      "--MW-- realized inference workload intensity on the IT-power basis"
  DATACENTER_TRAIN_SPATIAL(r,datacenter_train_day,rr,t) "--MWh/day-- source training work assigned to a destination region"
  DATACENTER_TRAIN_REFERENCE(r,allh,t) "--MW-- post-spatial training profile before temporal deferral"
  DATACENTER_TRAIN_FLEX(r,allh,t) "--MW-- temporally flexible training execution"
  DATACENTER_TRAIN_FLEX_FLOW(r,datacenter_train_route,t) "--MWh-- flexible training assigned directly from a source to a legal execution timeslice"
...
* Supply-curve investment exists only for eligible regions, modeled years
* strictly after the layout cutoff year, positive-capacity bins, and endogenous
* siting modes. datacenter_sc_capacity(r,"dc61") is itself zeroed in b_inputs.gms
* for regions with no priced-bin (dc1-dc60) capacity, so no separate bypass for
* the uncapped bin is needed here.
INV_DATACENTER_SC.fx(r,datacenter_sc_bin,t)$[
    (not datacenter_r(r))
    or (Sw_DatacenterDeploymentType=0)
    or (not tmodel_new(t))
    or (yeart(t)<=Sw_DatacenterLayoutYear)
    or (not datacenter_sc_capacity(r,datacenter_sc_bin))] = 0 ;
...
*  eq_load_flex_peak(r,allh,ccseason,t)    "--MWh-- adjust peak demand as needed based on the load flexibility (FLEX)"
 ...
 eq_datacenter_cap(r,t)                 "--MW-- CAP_DATACENTER accumulates INV_DATACENTER"
 eq_datacenter_state_fixed(r,t)         "--MW-- Data center IT load follows the exogenous BA path before endogenous layout or when fixed"
 eq_datacenter_state_floor(r,t)         "--MW-- Endogenous BA layout preserves IT load through the layout cutoff and retained exogenous growth"
 eq_datacenter_sc_inv(r,t)              "--MW-- endogenous additions are supplied by regional data center supply curves"
 eq_datacenter_sc_limit(r,datacenter_sc_bin,t) "--MW-- cumulative endogenous and retained-exogenous deployment cannot exceed bin capacity (skipped for the uncapped overflow bin)"
  eq_datacenter_country_siting(country,t) "--MW-- Endogenous data center IT load preserves country cumulative totals"
 eq_datacenter_total_siting(t)          "--MW-- Endogenous data center IT load preserves combined cumulative totals"
 eq_datacenter_base_train(r,allh,t)     "--MW-- baseline training workload intensity"
 eq_datacenter_base_infer(r,allh,t)     "--MW-- baseline inference workload intensity"
 eq_datacenter_infer_rigid(r,allh,t) "--MW-- inference workload is rigid"
 eq_datacenter_train_spatial_conservation(r,datacenter_train_day,t) "--MWh/day-- daily source training work is assigned exactly once"
 eq_datacenter_train_spatial_share(r,datacenter_train_day,t) "--MWh/day-- daily interregional training allocation limit"
 eq_datacenter_train_reference(r,datacenter_train_day,allh,t) "--MWh-- daily spatial allocation becomes destination reference MW"
 eq_datacenter_train_realized(r,datacenter_train_day,allh,t) "--MW-- realized training includes fixed and flexible portions"
 eq_datacenter_train_flex_source(r,allh,t) "--MWh-- every flexible source task is assigned exactly once"
 eq_datacenter_train_flex_destination(r,allh,t) "--MWh-- destination flexible training equals its assigned task energy"
 eq_datacenter_workload_cap(r,allh,t) "--MW-- shared active server capacity for realized workloads"
...
*[plus] Data center total electric load from optimized IT load placement
    + (
          datacenter_pue_r(r,t) * (
              datacenter_rho_idle * CAP_DATACENTER(r,t)
            + DATACENTER_TRAIN(r,h,t)
            + DATACENTER_INFER(r,h,t)
          )
      )$datacenter_r(r)
;
...
* Data center IT load capacity accumulates from model-year additions
eq_datacenter_cap(r,t)
    $[tmodel(t)
    $datacenter_r(r)
    $(not Sw_PCM)]..
...
* ---------------------------------------------------------------------------
* Through the layout cutoff year, or when deployment type is fixed, each
* state/province follows the exogenous cumulative IT load path.
eq_datacenter_state_fixed(r,t)
    $[tmodel(t)
    $datacenter_r(r)
    $[((Sw_DatacenterDeploymentType=0) or (yeart(t)<=Sw_DatacenterLayoutYear))]
    $(not Sw_PCM)]..
    ...
    datacenter_it_exog_r(r,t)
;

* ---------------------------------------------------------------------------
* IT load through the layout cutoff year cannot be relocated when endogenous layout begins,
* and the exogenous share of later growth must remain on the exogenous path.
eq_datacenter_state_floor(r,t)
    $[tmodel(t)
    $datacenter_r(r)
    $(Sw_DatacenterDeploymentType>0)
    $(yeart(t)>Sw_DatacenterLayoutYear)
    $(not Sw_PCM)]..
    ...
    datacenter_it_min_r(r,t)
    + (1 - Sw_DatacenterEndogenousShare)
      * (datacenter_it_exog_r(r,t) - datacenter_it_min_r(r,t))
;

* ---------------------------------------------------------------------------
* After the layout cutoff year, the retained exogenous increment stays at its
* prescribed regional floor. Both it and the endogenous increment occupy the
* regional supply curve, while only the endogenous increment incurs CAPEX.
eq_datacenter_sc_inv(r,t)
    $[tmodel(t)
    $datacenter_r(r)
    $(Sw_DatacenterDeploymentType>0)
    $(yeart(t)>Sw_DatacenterLayoutYear)
    ...
    + (1 - Sw_DatacenterEndogenousShare)
      * (datacenter_it_exog_r(r,t) - datacenter_it_min_r(r,t))
    - sum{tt$[tprev(t,tt)$(yeart(tt)<=Sw_DatacenterLayoutYear)],
        datacenter_it_exog_r(r,tt) }
    - sum{tt$[tprev(t,tt)$(yeart(tt)>Sw_DatacenterLayoutYear)],
        datacenter_it_min_r(r,tt)
        + (1 - Sw_DatacenterEndogenousShare)
          * (datacenter_it_exog_r(r,tt) - datacenter_it_min_r(r,tt)) }
    + sum{datacenter_sc_bin, INV_DATACENTER_SC(r,datacenter_sc_bin,t) }
;

eq_datacenter_sc_limit(r,datacenter_sc_bin,t)
    $[tmodel(t)
    $datacenter_r(r)
    $(Sw_DatacenterDeploymentType>0)
    $(yeart(t)>Sw_DatacenterLayoutYear)
    $(not Sw_PCM)
    $(not datacenter_sc_bin_uncapped(datacenter_sc_bin))]..

    sum{tt$[(tfix(tt) or tmodel(tt))
           $(yeart(tt)>Sw_DatacenterLayoutYear)
           $(yeart(tt)<=yeart(t))],
        INV_DATACENTER_SC(r,datacenter_sc_bin,tt)
        + datacenter_sc_exog_inv(r,datacenter_sc_bin,tt) }
...
* ---------------------------------------------------------------------------
* Deployment type 1 allows siting within each country while preserving country
* cumulative IT load totals.
eq_datacenter_country_siting(country,t)
    $[tmodel(t)
    $datacenter_country(country)
    $(Sw_DatacenterDeploymentType=1)
    $(yeart(t)>Sw_DatacenterLayoutYear)
    $(not Sw_PCM)]..

    sum{r$[datacenter_r(r)$r_country(r,country)], CAP_DATACENTER(r,t) }

    =e=

    datacenter_country_it_exog(country,t)
;

* ---------------------------------------------------------------------------
* Deployment type 2 allows siting across countries while preserving the combined
* cumulative IT load total.
eq_datacenter_total_siting(t)
    $[tmodel(t)
    $(Sw_DatacenterDeploymentType=2)
    $(not Sw_DatacenterNoBuild)
    $(yeart(t)>Sw_DatacenterLayoutYear)
    $(not Sw_PCM)]..

    sum{r$datacenter_r(r), CAP_DATACENTER(r,t) }

    =e=

    datacenter_total_it_exog(t)
;

* ---------------------------------------------------------------------------
* ---------------------------------------------------------------------------
* Baseline training and inference workload intensities are maintained on the
* IT-power basis (before PUE) so that future flexibility can act on each task.
eq_datacenter_base_train(r,h,t)
    $[tmodel(t)$datacenter_r(r)]..

    DATACENTER_BASE_TRAIN(r,h,t)

    =e=

    (datacenter_rho_max - datacenter_rho_idle)
    * CAP_DATACENTER(r,t)
    * datacenter_train_infer_util(t,"train_share")
    * datacenter_train_infer_util(t,"train_util")
;

* ---------------------------------------------------------------------------
eq_datacenter_base_infer(r,h,t)
    $[tmodel(t)$datacenter_r(r)]..

    DATACENTER_BASE_INFER(r,h,t)

    =e=

    (datacenter_rho_max - datacenter_rho_idle)
    * CAP_DATACENTER(r,t)
    * datacenter_train_infer_util(t,"infer_share")
    * datacenter_train_infer_util(t,"infer_util")
    * datacenter_infer_multiplier(r,h)
;

* Inference is latency-sensitive rigid load. Training first moves at a daily
* MWh scale, then the destination region schedules its eligible share in time.
eq_datacenter_infer_rigid(r,h,t)
    $[tmodel(t)$datacenter_r(r)]..
    DATACENTER_INFER(r,h,t) =e= DATACENTER_BASE_INFER(r,h,t)
;

eq_datacenter_train_spatial_conservation(r,datacenter_train_day,t)
    $[tmodel(t)$datacenter_r(r)
    $sum{h$datacenter_train_day_h(datacenter_train_day,h), 1}]..
    sum{rr$[datacenter_r(rr)$datacenter_spatial_route(r,rr)],
        DATACENTER_TRAIN_SPATIAL(r,datacenter_train_day,rr,t)}
    =e= sum{h$datacenter_train_day_h(datacenter_train_day,h),
        hours_daily(h) * DATACENTER_BASE_TRAIN(r,h,t)}
;

eq_datacenter_train_spatial_share(r,datacenter_train_day,t)
    $[tmodel(t)$datacenter_r(r)
    $sum{h$datacenter_train_day_h(datacenter_train_day,h), 1}]..
    sum{rr$[datacenter_r(rr)$datacenter_spatial_route(r,rr)$(not sameas(r,rr))],
        DATACENTER_TRAIN_SPATIAL(r,datacenter_train_day,rr,t)}
    =l= Sw_DatacenterSpatialFlexShare
        * sum{h$datacenter_train_day_h(datacenter_train_day,h),
            hours_daily(h) * DATACENTER_BASE_TRAIN(r,h,t)}
;

eq_datacenter_train_reference(r,datacenter_train_day,h,t)
    $[tmodel(t)$datacenter_r(r)$datacenter_train_day_h(datacenter_train_day,h)]..
    hours_daily(h) * DATACENTER_TRAIN_REFERENCE(r,h,t)
    =e= sum{rr$[datacenter_r(rr)$datacenter_spatial_route(rr,r)],
        datacenter_train_intraday_share(rr,datacenter_train_day,h)
        * DATACENTER_TRAIN_SPATIAL(rr,datacenter_train_day,r,t)}
;

eq_datacenter_train_realized(r,datacenter_train_day,h,t)
    $[tmodel(t)$datacenter_r(r)$datacenter_train_day_h(datacenter_train_day,h)]..
    DATACENTER_TRAIN(r,h,t)
    =e= (1 - Sw_DatacenterTrainTemporalFlexShare)
        * DATACENTER_TRAIN_REFERENCE(r,h,t)
        + DATACENTER_TRAIN_FLEX(r,h,t)
;

eq_datacenter_train_flex_source(r,h,t)
    $[tmodel(t)$datacenter_r(r)$sum{datacenter_train_route$datacenter_train_route_source(datacenter_train_route,h), 1}]..
    sum{datacenter_train_route$datacenter_train_route_source(datacenter_train_route,h),
        DATACENTER_TRAIN_FLEX_FLOW(r,datacenter_train_route,t)}
    =e= Sw_DatacenterTrainTemporalFlexShare
        * hours_daily(h) * DATACENTER_TRAIN_REFERENCE(r,h,t)
;

* Each route is generated in Python as a direct, forward path in a representative
* day or complete stress-period cycle. This prevents chained deferrals.
eq_datacenter_train_flex_destination(r,h,t)
    $[tmodel(t)$datacenter_r(r)$sum{datacenter_train_route$datacenter_train_route_destination(datacenter_train_route,h), 1}]..
    hours_daily(h) * DATACENTER_TRAIN_FLEX(r,h,t)
    =e= sum{datacenter_train_route$datacenter_train_route_destination(datacenter_train_route,h),
        DATACENTER_TRAIN_FLEX_FLOW(r,datacenter_train_route,t)}
;

eq_datacenter_workload_cap(r,h,t)
    $[tmodel(t)$datacenter_r(r)]..
    DATACENTER_TRAIN(r,h,t) + DATACENTER_INFER(r,h,t)
    =l= (datacenter_rho_max - datacenter_rho_idle) * CAP_DATACENTER(r,t)
;
...
* [plus] data center total electric load from optimized IT load placement
     + sum{h$h_ccseason_prm(h,ccseason),
           datacenter_pue_r(r,t) * (
             datacenter_rho_idle * CAP_DATACENTER(r,t)
             + DATACENTER_TRAIN(r,h,t)
             + DATACENTER_INFER(r,h,t)
...
eq_CAP_CONVERTER(r,t)
    ...
    + cap_converter_init(r)

    + cap_converter_fut(r,t)
