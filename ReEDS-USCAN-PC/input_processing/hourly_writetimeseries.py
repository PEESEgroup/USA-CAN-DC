...
def main(sw, reeds_path, inputs_case, periodtype='rep', make_plots=1):
    ...
    # %%### Legacy seasonal Canadian imports/exports
    if (
        os.path.exists(os.path.join(inputs_case, "can_exports_szn_frac.csv"))
        and os.path.exists(os.path.join(inputs_case, "can_imports_quarter_frac.csv"))
    ):
        ...
        can_exports_h_frac = pd.DataFrame(columns=["h", "frac_weighted"])
        can_imports_szn_frac = pd.DataFrame(columns=["szn", "frac_weighted"])
    ...
    ## Get last unique hydcf year by removing duplicated future fill data
    lastdatayr = (
        cf_hyd.pivot_table(index='t', columns=['*i','month','r'], values='cf_month')
        ## remove all duplicated data, leaving the first repeated future fill year
        .drop_duplicates()
        .index.max()
    )
