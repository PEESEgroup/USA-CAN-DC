...
def _is_active_value(val):
    """Interpret GDX set membership values (including c_bool wrappers)."""
    if pd.isna(val):
        return False
    if isinstance(val, (bool, int, float)):
        return bool(val)
    sval = str(val).strip().lower()
    if sval in {'false', '0', '0.0', 'c_bool(false)', 'no'}:
        return False
    if sval in {'true', '1', '1.0', 'c_bool(true)', 'yes'}:
        return True
    try:
        return float(sval) != 0.0
    except ValueError:
        # Fallback: treat non-empty/non-false strings as active.
        return True


def _active_rows(df):
    """Keep only active set members when Value is present."""
    if 'Value' not in df.columns:
        return df
    return df.loc[df['Value'].map(_is_active_value)].copy()
...
def check_param(param, kwargs, inputs, sw):
    ...
    # indexsets/excludesets are set memberships; filter out inactive elements.
    for subset in kwargs['indexsets'] + kwargs.get('excludesets', []):
        dfs[subset] = _active_rows(dfs[subset])
