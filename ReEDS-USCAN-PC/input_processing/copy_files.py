...
def read_runfiles(reeds_path, inputs_case, sw, agglevel_variables):
    ...
    try:
        runfiles.to_csv(
            os.path.join(inputs_case, 'runfiles_with_required.csv'),
            index=False,
        )
    except Exception as err:
        print(f"[Warning] Could not write runfiles_with_required.csv: {err}")
...
def forwardfill_hydcf(hydcf_in, sw):
    """
    Fills annual hydropower capacity factor data up to the ReEDS model end year
    using the same-month average from the available years in hydcf.csv.
    """
    df = hydcf_in.copy()
    # Get last data year and construct a new index for any required future years.
    lastdatayr = df.index.max()
    future_years = np.arange(lastdatayr+1, int(sw.endyear)+1).tolist()
    if future_years:
        same_month_average = df.mean(axis=0)
        future_df = pd.DataFrame(
            np.tile(same_month_average.to_numpy(), (len(future_years), 1)),
            index=future_years,
            columns=df.columns,
        )
        future_df.index.name = df.index.name
...
def get_regions_and_agglevel(
    ...
    if not NARIS:
        # Keep non-USA regions if the user explicitly requests country/CAN in GSw_Region.
        keep_non_usa = False
        gswr = str(sw.get('GSw_Region', ''))
        if '/' in gswr:
            region_groups = gswr.split('//') if '//' in gswr else [gswr]
            for group in region_groups:
                try:
                    level, regions = group.split('/')
                except ValueError:
                    continue
                if level.strip().lower() == 'country':
                    if any(r.strip().upper() == 'CAN' for r in regions.split('.')):
                        keep_non_usa = True
                        break
        if not keep_non_usa:
    ...
    # Backfill a Canada-US pooling layer for hierarchy files that predate the
    # explicit canus_pool column. Quebec is grouped with eastern regions.
    if 'canus_pool' not in hierarchy.columns and 'interconnect' in hierarchy.columns:
        hierarchy['canus_pool'] = hierarchy['interconnect'].replace({'quebec': 'eastern'})
    if 'whole_pool' not in hierarchy.columns:
        hierarchy['whole_pool'] = 'All'
    ...
    for level in ['ba','aggreg']:
        mask = hier_sub['resolution'] == level
        hier_sub.loc[mask, 'r'] = hier_sub.loc[mask, level]
...
def subset_to_valid_regions(
    ...
    if (agglevel_variables['lvl'] == 'mult') and ('lvl' in filepath) and (not sc_point_gid_index):
        ...
        match filename:        
            case 'hydcf.csv':
                # Fill data to the endyear using same-month average capacity factors
