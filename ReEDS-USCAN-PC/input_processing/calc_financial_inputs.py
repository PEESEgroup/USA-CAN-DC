...
def calc_financial_inputs(inputs_case):
    ...
    #%%### Calculate financial multipliers for data-center supply-curve CAPEX
    # Data centers use general system financing with a one-year construction
    # schedule, 15-year tax depreciation, no tax credits, and a 40-year
    # capital recovery period.
    dfdc = financials_sys.dropna(how='any').copy()
    dfdc['eval_period'] = 40
    dfdc['construction_sch'] = '1'
    dfdc['depreciation_sch'] = 15
    dfdc['finance_diff_real'] = 0.0
    dfdc['itc_frac'] = 0.0
    dfdc['itc_tax_equity_penalty'] = 0.0
    dfdc['itc_frac_monetized'] = 0.0
    dfdc['annual_degradation'] = 0.0
    dfdc = reeds.financials.calc_financial_multipliers(
        df_inv=dfdc, construction_schedules=construction_schedules,
        depreciation_schedules=depreciation_schedules, timetype=sw['timetype'],
    )
    dfdc['crf_tech'] = reeds.financials.calc_crf(dfdc['d_real'], dfdc['eval_period'])
    dfdc['cap_cost_mult'] = reeds.financials.calc_final_capital_cost_multiplier(dfdc)
    dfdc.rename(columns={'t':'*t'})[['*t','cap_cost_mult']].round(6).to_csv(
        os.path.join(inputs_case, 'datacenter_cap_cost_mult.csv'), index=False)
