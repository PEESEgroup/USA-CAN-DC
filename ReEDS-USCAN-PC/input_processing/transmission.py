...
def get_cap_converter_init(valid_regions):
    """Build initial VSC converter capacity [MW] by summing incident existing VSC lines."""
    trancap_init_nonac = pd.read_csv(os.path.join(inputs_case, 'transmission_capacity_init_nonAC.csv'))
    vsc_init = trancap_init_nonac.loc[trancap_init_nonac['trtype'] == 'VSC', ['r', 'rr', 'MW']].copy()
    if vsc_init.empty:
        return pd.DataFrame(columns=['*r', 'MW'])

    endpoints = pd.concat(
        [
            vsc_init[['r', 'MW']].rename(columns={'r': 'r'}),
            vsc_init[['rr', 'MW']].rename(columns={'rr': 'r'}),
        ],
        axis=0,
        ignore_index=True,
    )
    cap_converter_init = (
        endpoints.loc[endpoints['r'].isin(valid_regions['r'])]
        .groupby('r', as_index=False)['MW']
        .sum()
        .rename(columns={'r': '*r'})
        .sort_values('*r')
    )
    return cap_converter_init


def get_cap_converter_fut(valid_regions, trancap_fut):
    """Build certain future VSC converter capacity [MW] indexed by commissioning year.

    Each certain VSC line contributes its MW to both endpoint nodes starting from
    the year the line is commissioned. GAMS will accumulate these into a cumulative
    parameter so that capacity persists in all subsequent model years.
    """
    vsc_certain = trancap_fut.loc[
        (trancap_fut['status'].str.capitalize() == 'Certain')
        & (trancap_fut['trtype'] == 'VSC'),
        ['r', 'rr', 't', 'MW'],
    ].copy()
    if vsc_certain.empty:
        return pd.DataFrame(columns=['*r', 't', 'MW'])

    endpoints = pd.concat(
        [
            vsc_certain[['r', 't', 'MW']],
            vsc_certain[['rr', 't', 'MW']].rename(columns={'rr': 'r'}),
        ],
        axis=0,
        ignore_index=True,
    )
    cap_converter_fut = (
        endpoints.loc[endpoints['r'].isin(valid_regions['r'])]
        .groupby(['r', 't'], as_index=False)['MW']
        .sum()
        .rename(columns={'r': '*r'})
        .sort_values(['*r', 't'])
    )
    return cap_converter_fut
...
cap_converter_init = get_cap_converter_init(valid_regions)
cap_converter_init.round(3).to_csv(
    os.path.join(inputs_case, 'cap_converter_init.csv'),
    index=False,
)
...
cap_converter_fut = get_cap_converter_fut(valid_regions, trancap_fut)
cap_converter_fut.to_csv(
    os.path.join(inputs_case, 'cap_converter_fut.csv'),
    index=False,
)
