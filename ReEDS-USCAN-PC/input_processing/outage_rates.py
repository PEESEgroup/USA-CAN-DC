...
def aggregate_regions(df, inputs_case):
    ...
    if 'aggreg' in agglevel_variables['agglevel']:
        ...
        dfout.columns = pd.MultiIndex.from_tuples(
            [(x[0], r2aggreg[x[1]]) for x in dfout.columns],
            names=dfout.columns.names,
        )
        dfout = dfout.T.groupby(level=[techcol, 'r']).mean().T
...
def write_outages(df, h5path):
    ...
    with h5py.File(h5path, 'w') as f:
        f.create_dataset('index', data=df.index.astype(str).str.encode('utf-8'), dtype='S29')
