...
function get_forced_outage_data(data::ReEDSdatapaths)
    ...
    df = DataFrames.rename!(df, ["ResourceType", "FOR"])
    df[!, "ResourceType"] = lowercase.(string.(df[!, "ResourceType"]))
    return df
end
...
function get_hydro_data(data::ReEDSdatapaths)
    ...
    hydcf.i = lowercase.(strip.(string.(hydcf.i)))
    hydcf.r = strip.(string.(hydcf.r))
    hydcf.month = uppercase.(strip.(string.(hydcf.month)))
    ...
    hydcapadj.i = lowercase.(strip.(string.(hydcapadj.i)))
    hydcapadj.r = strip.(string.(hydcapadj.r))
    hydcapadj.month = uppercase.(strip.(string.(hydcapadj.month)))
