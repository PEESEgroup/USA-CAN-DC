    ...
    for row in eachrow(thermal_builds)
        ...
        elseif lowercase(tech) in keys(FOR_dict)
            ...
            @info(
                ...
                "static value of $(FOR_dict[lowercase(tech)]) from outage_forced_static.csv"
            )
        else
            @error(
...
function process_hydro(
    ...
    for (idx, row) in enumerate(DataFrames.eachrow(disp_combined_caps))
        ...
        for monhr in monthhours
            try
                ...
                month_energy = filter(x -> (x.month == monhr.month), reg_plant_subset)
                if isempty(month_energy)
                    throw(ArgumentError("Missing hydcf for $(row.r),$(row.i),$(monhr.month)"))
                end
                monthly_energy[first(reqd_slice)] =
                    (monhr.numhrs * month_energy[:, :value] * row.MW_sum)[1]
                ...
                month_adjust = filter(x -> (x.month == monhr.month), capacity_adjust)
                if isempty(month_adjust)
                    throw(ArgumentError("Missing hydcapadj for $(row.r),$(row.i),$(monhr.month)"))
                end
                dispatch_limit[reqd_slice] .=
                    (month_adjust[:, :value] * row.MW_sum)[1]
            catch e
    ...
    for (idx, row) in enumerate(DataFrames.eachrow(non_disp_combined_caps))
        ...
        for monhr in monthhours
            try
                reqd_slice = monhr.slice
                month_cf = filter(x -> (x.month == monhr.month), reg_type)
                if isempty(month_cf)
                    throw(ArgumentError("Missing hydcf for $(row.r),$(row.i),$(monhr.month)"))
                end
                hourly_capacity[reqd_slice] .=
                    (month_cf[:, :value] * row.MW_sum)[1]
            catch e
                if isa(e, BoundsError) || isa(e, ArgumentError)
                    @error "$(row.r),$(row.i),$(e)"
                else
